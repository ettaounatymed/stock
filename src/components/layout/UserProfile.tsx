...
export function UserProfile({
  ...
  return (
    <article className="w-full rounded-3xl border border-white/10 bg-white/10 p-8 shadow-2xl shadow-black/30 backdrop-blur-xl sm:p-10">
      <p className="text-sm uppercase tracking-[0.35em] text-cyan-200">Private access</p>
      <h1 className="mt-3 text-3xl font-semibold text-white sm:text-4xl">🔐 Sign in to open the stock tracker</h1>
      <p className="mt-4 max-w-2xl text-slate-200/90">Sign in with your Supabase account. Owners can manage records; viewer accounts can consult products, statistics, and purchase history.</p>
      ...
      {isSupabaseConfigured ? (
        ...
        <div className="mt-8 rounded-3xl border border-amber-400/20 bg-amber-400/10 p-4 text-sm text-amber-50">
          Supabase is not configured yet. Add your project URL and anon key in the environment variables to enable private access.
        </div>
      )}

      <p className="mt-4 text-sm text-slate-200/90">{authStatus || "Use the email and password created in your Supabase project."}</p>
    </article>
  );
}